...
func main() {
	setupPrus()
	go releaseToken()
	play("/mnt/sdcard/Render_08-11-22_adafruit.led", 0, 600, 25)
	//runServer()
}
...
func setupPrus() {
	...
	if err := prus.Unit(0).LoadAt(pru0Code, 0x00); err != nil {
		log.Fatalf("Failed to load PRU0 code: %s\n", err)
	}

	if err := prus.Unit(1).LoadAt(pru1Code, 0x00); err != nil {
...
func outputFrame(input string, frameIdx uint32) error {
	...
	pru1Ram := PruBuffer{addr: 0x0000_0000, size: 8 * 1024, data: prus.Unit(1).Ram, next: &sharedRam}
	sharedRam.next = &pru1Ram
	buffers := []PruBuffer{pru1Ram, sharedRam}
	...
	if prus.Unit(0).IsRunning() {
		return fmt.Errorf("Recorder timed out")
	}
	samples := prus.Unit(0).Ram[4 : 4*4*28+4]
